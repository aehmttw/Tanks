...
public class LWJGLWindow extends BaseWindow
{
    ...
    {
        String result = null;
        try (InputStream in = this.getResource(fileName); Scanner scanner = new Scanner(in, java.nio.charset.StandardCharsets.UTF_8.name()))
        {
    ...
    public void setUpPerspective()
    {
        ...
        else
        {
            if (this.orthographic)
                ...
                glFrustum(-absoluteWidth / (absoluteDepth * 2.0) * m, absoluteWidth / (absoluteDepth * 2.0) * m, absoluteHeight / (absoluteDepth * 2.0) * m,
                        -absoluteHeight / (absoluteDepth * 2.0) * m, m, absoluteDepth * m * clipDistMultiplier);
        }
