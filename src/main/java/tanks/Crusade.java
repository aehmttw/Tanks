...
public class Crusade
{
        ...
        if (ScreenPartyHost.isServer)
        {
            for (ServerHandler sh: ScreenPartyHost.server.connections)
            {
                if (sh.player != null)
                {
                    ...
                    for (int in = 0; in < sh.player.hotbar.itemBar.slots.length; in++) sh.queueEvent(new EventSetItem(sh.player, in, sh.player.hotbar.itemBar.slots[in]));
    ...
    {
        for (int i = performances.size(); i < currentLevel; i++) performances.add(new LevelPerformance(i));
