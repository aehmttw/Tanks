...
public class Drawing
{
     ...
     * 0: default
     * <p>
     * +1 hide behind face +2 hide front face +4 hide bottom face +8 hide top face +16 hide left face +32 hide right face
     * <p>
     * +64 draw on top
     */
    ...
    {
        double drawX = interfaceScale * (x - sizeX / 2 - lineWidth) + Math.max(0, Panel.windowWidth - interfaceSizeX * interfaceScale) / 2;
        double drawY = interfaceScale * (y - sizeY / 2 - lineWidth) + Math.max(0, Panel.windowHeight - statsHeight - interfaceSizeY * interfaceScale) / 2;
        double drawSizeX = sizeX * interfaceScale;
        double drawSizeY = sizeY * interfaceScale;
    ...
    public void drawTooltip(String[] text, double x, double y)
    {
        ...
        double endX = (Game.game.window.absoluteWidth / Drawing.drawing.interfaceScale - Drawing.drawing.interfaceSizeX) / 2 + Drawing.drawing.interfaceSizeX
    ...
    public void drawPopup(double x, double y, double sX, double sY, double borderWidth, double borderRadius)
    {
        ...
        // drawInterfaceRect(x + borderWidth, y + borderWidth, sX, sY, borderWidth,
        // borderRadius);
    ...
    public double gameToInterfaceCoordsX(double x)
    {
        ...
        /*
         * double rawX = (x + getPlayerMouseOffsetX()) * scale; rawX += (Drawing.drawing.interfaceSizeX - sizeX * scale / interfaceScale) / 2 * interfaceScale; double x1 = rawX /
         * interfaceScale;
         *
         * if (enableMovingCamera && movingCamera && enableMovingCameraX) x1 -= (Game.game.window.absoluteWidth - interfaceScale * interfaceSizeX) / 2 / interfaceScale;
         *
         * return x1;
         */
    }
    ...
    public double gameToInterfaceCoordsY(double y)
    {
        ...
        /*
         * double rawY = (y + getPlayerMouseOffsetY()) * scale; rawY += (Drawing.drawing.interfaceSizeY - sizeY * scale / interfaceScale) / 2 * interfaceScale; double y1 = rawY /
         * interfaceScale;
         *
         * if (enableMovingCamera && movingCamera && enableMovingCameraY) y1 -= (Game.game.window.absoluteHeight - interfaceScale * interfaceSizeY - statsHeight) / 2 /
         * interfaceScale;
         *
         * return y1;
         */
    ...
    /*
     * public void setScreenSize(int x, int y) { sizeX = x; sizeY = y; this.setSize((int) (x * scale), (int) ((y) * scale)); }
     */
    ...
    public double getPlayerOffsetX()
    {
        ...
        {
            double frac = Math.min(1,
                    (Panel.panel.age - getTrackOffset() - Panel.panel.pastPlayerTime.get(0)) / (Panel.panel.pastPlayerTime.get(1) - Panel.panel.pastPlayerTime.get(0)));
    ...
    public double getPlayerOffsetY()
    {
        ...
        double margin = Math.max(0, Math.min(Game.tile_size * 2, Game.currentSizeY * Game.tile_size * Drawing.drawing.scale - (Panel.windowHeight - Drawing.drawing.statsHeight)))
                / 2;
    ...
    {
        if (right)
            return (Game.game.window.absoluteWidth / Drawing.drawing.interfaceScale - Drawing.drawing.interfaceSizeX) / 2 + Drawing.drawing.interfaceSizeX
                    - Game.game.window.getEdgeBounds() / Drawing.drawing.interfaceScale;
        else
            return (Game.game.window.absoluteWidth / Drawing.drawing.interfaceScale - Drawing.drawing.interfaceSizeX) / 2 + Drawing.drawing.interfaceSizeX
                    - Game.game.window.getEdgeBounds() / Drawing.drawing.interfaceScale;
    ...
    public boolean isOutOfBounds(double drawX, double drawY)
    {
        ...
        {
            // return (drawX - gameToAbsoluteX(Game.playerTank.posX, 0)) *
            // Math.cos(Game.playerTank.angle)
            // + (drawY - gameToAbsoluteY(Game.playerTank.posY, 0)) *
            // Math.sin(Game.playerTank.angle) < -dist;
            return false;
        }
    }
