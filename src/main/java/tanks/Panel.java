...
public class Panel
{
    ...
    public void update()
    {
        ...
        if (!started && (Game.game.window.validPressedKeys.contains(InputCodes.KEY_F) || !Game.cinematic))
        {
            ...
            // Drawing.drawing.playMusic("menu_intro.ogg", Game.musicVolume, false, "intro",
            // 0, false);
        }
                ...
                if (!(Game.eventsIn.get(i) instanceof IOnlineServerEvent))
                {
                    ...
                    try
                    {
                        if (e instanceof IStackableEvent)
                            stackedEventsIn.put(IStackableEvent.f(NetworkEventMap.get(e.getClass()) + IStackableEvent.f(((IStackableEvent) e).getIdentifier())),
                                    (IStackableEvent) e);
                        else
        ...
        if (Game.screen instanceof ScreenGame && Drawing.drawing.enableMovingCamera && Panel.zoomTarget >= 0
                && (((ScreenGame) Game.screen).spectatingTank != null || (Game.playerTank != null && !Game.playerTank.destroy)) && ((ScreenGame) Game.screen).playing)
        {
    ...
    public void draw()
    {
        if ((Game.game.window.drawingShadow || !Game.shadowsEnabled)
                && (Game.screen instanceof ScreenGame && !(((ScreenGame) Game.screen).paused && !ScreenPartyHost.isServer && !ScreenPartyLobby.isClient)))
        ...
        if (continuation == null)
        {
            ...
            if (!(Game.screen instanceof ScreenGame))
            {
                ...
                Drawing.drawing.unzoomedScale = Drawing.drawing.scale;
                Drawing.drawing.scale = Game.screen.getScale();
                Drawing.drawing.interfaceScale = Drawing.drawing.interfaceScaleZoom * Math.min(Panel.windowWidth / 28, (Panel.windowHeight - Drawing.drawing.statsHeight) / 18)
                        / 50.0;
                Game.game.window.absoluteDepth = Drawing.drawing.interfaceScale * Game.absoluteDepthBase;
            }
    ...
    public void drawBar(double offset)
    {
        ...
        Game.game.window.fontRenderer.drawString(boundary + 150, offset + (int) (Panel.windowHeight - 40 + 22), 0.4, 0.4,
                "Memory used: " + used / 1048576 + "/" + total / 1048576 + "MB (" + Math.round(allocatedLastSecond / 104857.6) / 10. + " MB/s)");
