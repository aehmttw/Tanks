...
public class Bullet extends Movable implements ICopyable<Bullet>, ITanksONEditable
{
    ...
    // @Property(id = "area_effect", name = "Area effect", category =
    // BulletPropertyCategory.impact)
    ...
    /**
     * If bullet should return to original speed if blown to a slower speed by
     * something like wind
     */
    ...
    // Whether this bullet class can deflect other bullets (things like arc and air
    // strike can't)
    public boolean canDeflect = true;

    /**
     * Do not use if you plan to place this bullet in the game field. Only for
     * templates. Use another constructor if you want to add the bullet to the game
     * field.
     */
    public Bullet()
    {
    ...
    public Bullet rebound(Movable m)
    {
        try
        {
            Bullet b = this.getClass().getConstructor(double.class, double.class, Tank.class, boolean.class, ItemBullet.ItemStackBullet.class).newInstance(m.posX, m.posY,
                    this.tank, false, this.item);
    ...
    /**
     * Triggered when colliding with an obstacle or map edge and having no bounces
     * left
     */
    ...
    /**
     * When 2 bullets that knock bullets collide, an elastic collision takes place,
     * with mass scaling by bullet knockback.
     */
    ...
    public void update()
    {
        ...
        if (!this.previousRebounds.isEmpty() && !this.beganRebound)
        {
            ...
            for (Movable m1 : Game.movables)
            {
                ...
                if (m1 instanceof Tank && this.isHarmful() != Team.isAllied(this, m1))
                    eligible = true;
                else if ((m1 instanceof Bullet && ((Bullet) m1).enableCollision && ((Bullet) m1).bulletCollision && ((Bullet) m1).delay <= 0) && this.bulletCollision
                        && !Team.isAllied(this, m1))
                    eligible = true;
        ...
        if (((this.age > lifespan && this.lifespan > 0) || (this.range > 0 && Math.pow(this.originX - this.posX, 2) + Math.pow(this.originY - posY, 2) > this.range * this.range))
                && !this.destroy)
        {
            this.pop();
            this.collisionX = this.posX;
    ...
    {
        if (!this.effect.trailEffects.isEmpty() && !this.addedTrail && !this.destroy && (GameObject.absoluteAngleBetween(this.getPolarDirection(), this.lastTrailAngle) >= 0.001
                || (this.trail3d && GameObject.absoluteAngleBetween(this.getPolarPitch(), this.lastTrailPitch) >= 0.1)))
        {
            this.addTrail(true);
        }
    }
    ...
    public void addTrail(boolean redirect)
    {
        ...
        for (int i = 0; i < this.effect.trailEffects.size(); i++)
        {
            ...
            if (!this.trail3d || !Game.enable3d)
                this.addTrailObj(i,
                        new Trail(this, this.speed, x, y, this.size * speed / 3.125 * t.delay, this.size / 2 * t.backWidth, this.size / 2 * t.frontWidth,
                                this.size * speed / 3.125 * t.maxLength, this.lastTrailAngle, t.frontColor.red, t.frontColor.green, t.frontColor.blue, t.frontColor.alpha,
                                t.backColor.red, t.backColor.green, t.backColor.blue, t.backColor.alpha, t.glow, t.luminosity, t.frontCircle, t.backCircle),
                        redirect);
            else
                this.addTrailObj(i,
                        new Trail3D(this, this.speed, x, y, z, this.size * speed / 3.125 * t.delay, this.size / 2 * t.backWidth, this.size / 2 * t.frontWidth,
                                this.size * speed / 3.125 * t.maxLength, this.lastTrailAngle, this.lastTrailPitch, t.frontColor.red, t.frontColor.green, t.frontColor.blue,
                                t.frontColor.alpha, t.backColor.red, t.backColor.green, t.backColor.blue, t.backColor.alpha, t.glow, t.luminosity, t.frontCircle, t.backCircle),
                        redirect);
        }
    }
    ...
    public void drawHoming()
    {
        ...
        if (!ScreenGame.finishedQuick && this.homingTarget != null)
        {
            ...
            Drawing.drawing.drawImage(frac * Math.PI / 2 + this.getAngleInDirection(this.homingTarget.posX, this.homingTarget.posY), "cursor.png", this.homingTarget.posX,
                    this.homingTarget.posY, s, s);
    ...
    public void drawForInterface(double x, double width, double y, double size, ArrayList<Effect> effects, ArrayList<Effect> removeEffects, Random r, Color base, Color turret)
    {
        ...
        if (!this.effect.overrideGlowColor)
        {
            if (this.overrideOutlineColor)
                ...
                Drawing.drawing.setColor(turret.red * this.effect.glowIntensity, turret.green * this.effect.glowIntensity, turret.blue * this.effect.glowIntensity, 255,
                        this.effect.glowGlowy ? 1 : this.effect.luminance);
        } else
