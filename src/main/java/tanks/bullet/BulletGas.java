...
public class BulletGas extends Bullet implements IDrawableWithGlow
{
    ...
    public void drawForInterface(double x, double width, double y, double size, ArrayList<Effect> effects, ArrayList<Effect> removeEffects, Random r, Color base, Color turret)
    {
        ...
        for (int i = 0; i < (int) life; i++)
        {
            ...
            Drawing.drawing.setColor(c1.red * frac + c2.red * (1 - frac) + randoms[i] * this.noise.red, c1.green * frac + c2.green * (1 - frac) + randoms[i] * this.noise.green,
                    c1.blue * frac + c2.blue * (1 - frac) + randoms[i] * this.noise.blue, opacity, this.effect.luminance);
            Drawing.drawing.fillInterfaceOval(start + l * (1 - f), y + (randoms[(int) (life + i)] - 0.5) * spread * i / 100.0 * speed, (frac * startSize + (1 - frac) * endSize),
                    (frac * startSize + (1 - frac) * endSize));
        }
        ...
        for (int i = 0; i < (int) life; i++)
        {
            ...
            if (!this.effect.overrideGlowColor)
                Drawing.drawing.setColor(c1.red * frac + c2.red * (1 - frac) + randoms[i] * this.noise.red, c1.green * frac + c2.green * (1 - frac) + randoms[i] * this.noise.green,
                        c1.blue * frac + c2.blue * (1 - frac) + randoms[i] * this.noise.blue, opacity, opacity / 255 * this.effect.glowIntensity);
            else
                Drawing.drawing.setColor(this.effect.glowColor.red, this.effect.glowColor.green, this.effect.glowColor.blue, opacity, opacity / 255 * this.effect.glowIntensity);
