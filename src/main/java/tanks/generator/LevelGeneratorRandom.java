    ...
    public static String generateLevelString(int seed)
    {
            ...
            while (n > 0)
            {
                ...
                if (!cells[x][y])
                {
                    for (int i = Math.max(x - 2, 0); i <= Math.min(x + 2, width - 1); i++)
                        for (int j = Math.max(y - 2, 0); j <= Math.min(y + 2, height - 1); j++) cells[i][j] = true;
        ...
        for (int i = 0; i < numPlayers; i++)
        {
            ...
            for (int a = -bound; a <= bound; a++)
                for (int j = -bound; j <= bound; j++) cells[Math.max(0, Math.min(width - 1, x + a))][Math.max(0, Math.min(height - 1, y + j))] = true;
        ...
        for (int i = 0; i < numTanks; i++)
        {
            ...
            for (int a = -1; a <= 1; a++) for (int j = -1; j <= 1; j++) cells[Math.max(0, Math.min(width - 1, x + a))][Math.max(0, Math.min(height - 1, y + j))] = true;
