...
public class Button implements IDrawable, ITrigger
{
    ...
    public void draw()
    {
        ...
        if (this.model != null)
        {
            ...
            drawing.drawInterfaceModel2D(model, this.posX + this.imageXOffset, this.posY + this.imageYOffset, 0, this.imageSizeX * 0.75, this.imageSizeY * 0.75,
                    this.imageSizeY * 0.75);
        }
    ...
    public static void drawGlow(double posX, double posY, double sizeX, double sizeY, double size, double r, double g, double b, double a, boolean glow)
    {
        ...
        for (int i = 0; i < 30; i++)
        {
            ...
            drawing.addInterfaceVertex(posX + sizeX / 2 - sizeY / 2, posY, 0);
            drawing.setColor(r, g, b, 0);
            drawing.addInterfaceVertex(posX + sizeX / 2 - sizeY / 2 + sizeY * Math.cos((i + 45) / 30.0 * Math.PI) * size, posY + sizeY * Math.sin((i + 45) / 30.0 * Math.PI) * size,
                    0);
            drawing.addInterfaceVertex(posX + sizeX / 2 - sizeY / 2 + sizeY * Math.cos((i + 46) / 30.0 * Math.PI) * size, posY + sizeY * Math.sin((i + 46) / 30.0 * Math.PI) * size,
                    0);
        }
