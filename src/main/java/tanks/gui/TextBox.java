...
public class TextBox implements IDrawable, ITrigger
{
    ...
    public void drawInput()
    {
        ...
        if (Game.game.window.fontRenderer.getStringSizeX(Drawing.drawing.fontSize, inputText) / Drawing.drawing.interfaceScale > this.sizeX - 80)
            Drawing.drawing.setInterfaceFontSize(
                    size * (this.sizeX - 80) / (Game.game.window.fontRenderer.getStringSizeX(Drawing.drawing.fontSize, inputText) / Drawing.drawing.interfaceScale));
    ...
    public void update()
    {
        ...
        if (selected)
        {
            double frac = Math.max(0,
                    Math.round(Drawing.drawing.interfaceScale * (this.posY + 30)
                            + Math.max(0, Panel.windowHeight - Drawing.drawing.statsHeight - Drawing.drawing.interfaceSizeY * Drawing.drawing.interfaceScale) / 2)
                            - Game.game.window.absoluteHeight * Game.game.window.keyboardFraction)
    ...
    public boolean checkMouse(double mx, double my, boolean down, boolean valid, InputPoint p)
    {
        ...
        if (Game.game.window.touchscreen)
        {
            ...
            if (selected && valid && mx > posX + sizeX / 2 - sizeY * 2 && mx < posX + sizeX / 2 - sizeY && my > posY - sizeY / 2 - sizeY * 13 / 16
                    && my < posY + sizeY / 2 - sizeY * 13 / 16)
            {
                this.copy();
                handled = true;
                Drawing.drawing.playVibration("click");
    ...
    public static void drawTallGlow(double posX, double posY, double sizeX, double sizeY, double extra, double size, double r, double g, double b, double a, boolean glow)
    {
        ...
        for (int i = 0; i < 15; i++)
        {
            ...
            drawing.addInterfaceVertex(posX - sizeX / 2 + sizeY / 2, posY, 0);
            drawing.setColor(r, g, b, 0);
            drawing.addInterfaceVertex(posX - sizeX / 2 + sizeY / 2 + sizeY * Math.cos((i + 15) / 30.0 * Math.PI) * size, posY + sizeY * Math.sin((i + 15) / 30.0 * Math.PI) * size,
                    0);
            drawing.addInterfaceVertex(posX - sizeX / 2 + sizeY / 2 + sizeY * Math.cos((i + 16) / 30.0 * Math.PI) * size, posY + sizeY * Math.sin((i + 16) / 30.0 * Math.PI) * size,
                    0);
            ...
            drawing.addInterfaceVertex(posX + sizeX / 2 - sizeY / 2 + sizeY * Math.cos((i + 1) / 30.0 * Math.PI) * size, posY + sizeY * Math.sin((i + 1) / 30.0 * Math.PI) * size,
                    0);
