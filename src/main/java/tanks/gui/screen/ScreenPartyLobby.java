...
public class ScreenPartyLobby extends Screen
{
    ...
    Button previousUsernamePage = new Button(Drawing.drawing.interfaceSizeX / 2 + username_x_offset, Drawing.drawing.interfaceSizeY / 2 + username_y_offset, 300, 30,
            "Previous page", () -> usernamePage--);
    ...
    public void draw()
    {
        ...
        if (connections != null && connections.size() > 1)
            Drawing.drawing.displayInterfaceText(Drawing.drawing.interfaceSizeX / 2 + username_x_offset, Drawing.drawing.interfaceSizeY / 2 - 220, "%d players in this party:",
                    connections.size());
        else
        ...
        if (connections != null)
        {
            ...
            if (connections != null)
            {
                for (int i = (int) (this.usernamePage * entries_per_page + Math.signum(this.usernamePage)); i < Math.min(((this.usernamePage + 1) * entries_per_page + 1),
                        connections.size()); i++)
                {
