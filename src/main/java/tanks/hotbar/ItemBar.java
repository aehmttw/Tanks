...
public class ItemBar
{
    ...
    {
        for (int i = 0; i < slots.length; i++) slots[i] = new ItemEmpty.ItemStackEmpty();
    ...
    public boolean addItem(Item.ItemStack<?> item)
    {
        ...
        for (int x = 0; x < this.slots.length; x++)
        {
            if (this.slots[x].item.name.equals(i.item.name)
                    && ((this.slots[x].stackSize >= this.slots[x].maxStackSize && this.slots[x].maxStackSize > 0) || this.slots[x].stackSize == 0))
                continue;
    ...
    public void draw()
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            {
                double a = (selected >= 0 && selected < hotbarSlots && !(slots[selected].item instanceof ItemEmpty) && slots[selected].item.rightClick) ? 0.5 : 1;
                Drawing.drawing.setColor(slotSecondarySelectedR + eb, slotSecondarySelectedG + eb, slotSecondarySelectedB + eb,
                        (100 - this.player.hotbar.percentHidden) * 1.27 * a);
            } else if (i >= items)
    ...
    public void drawCircle()
    {
        ...
        TankPlayer p = ((TankPlayer) this.player.tank);
        int count = items + p.abilities.size();
        double opacity = player.hotbar.circleVisibility / player.hotbar.circleVisibilityMax
                * Math.min(1, player.hotbar.circlePersistenceVisibility + Math.max(0, (400 - timeSinceSwitch) / 200));
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            if (s != null)
            {
                ...
                if (s.stackSize > 0)
                {
                    ...
                    Drawing.drawing.drawInterfaceText(x + imgSize - count_margin_right * (imgSize / 50), y + extra + imgSize - count_margin_bottom * (imgSize / 50),
                            Integer.toString(s.stackSize), true);
                    Drawing.drawing.setColor(slotBgBrightness, slotBgBrightness, slotBgBrightness, slotBgA * opacity);
                }
            }
        }
