...
public class ItemBullet extends Item implements ICopyable<ItemBullet>
{
    ...
    public static class ItemStackBullet extends ItemStack<ItemBullet>
    {
        ...
        public void use(Tank m)
        {
            try
            {
                ...
                if (this.item.bullet.shotSound != null && this.item.bullet.soundVolume > 0)
                    Drawing.drawing.playGlobalSound(this.item.bullet.shotSound, (float) (this.item.bullet.pitch * (1 - (Math.random() * 0.5) * this.item.bullet.pitchVariation)),
                ...
                for (int i = 0; i < q; i++)
                {
                    ...
                    Bullet b = this.item.bullet.getClass().getConstructor(double.class, double.class, Tank.class, boolean.class, ItemStackBullet.class).newInstance(m.posX, m.posY,
                            m, true, this);
        ...
        {
            return t != null && (this.item.bullet.maxLiveBullets <= 0 || this.liveBullets <= this.item.bullet.maxLiveBullets - this.item.bullet.shotCount) && !(this.cooldown > 0);
        }
    }
}
