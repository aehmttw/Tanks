...
public class SteamWorkshopHandler
{
    ...
    public SteamUGCCallback workshopCallback = new SteamUGCCallback()
    {
        ...
        public void onSubmitItemUpdate(SteamPublishedFileID publishedFileID, boolean needsToAcceptWLA, SteamResult result)
        {
            ...
            {
                if (!needsToAcceptWLA)
                    Game.screen = new ScreenWorkshopActionResult(new ScreenSteamWorkshop(), "Uploaded!",
                            uploadingType + " was uploaded to Steam Workshop! \n Please note that it may take a few minutes for it to show up in listings.", true);
                else
                    Game.screen = new ScreenWorkshopMustAcceptAgreement(new ScreenSteamWorkshop(), "Uploaded!",
                            uploadingType + " was uploaded to Steam Workshop! \n However, you haven't yet accepted the Steam Workshop Legal agreement. \n Your "
                                    + uploadingType.toLowerCase() + " will be private until you do so.",
                            true);
        ...
        public void onDeleteItem(SteamPublishedFileID publishedFileID, SteamResult result)
        {
            if (result.equals(SteamResult.OK))
                Game.screen = new ScreenWorkshopActionResult(new ScreenSteamWorkshop(), "Success!",
                        uploadingType + " was removed from Steam Workshop! \n Please note that it may take a few minutes for it to disappear from listings.", true);
            else
                Game.screen = new ScreenWorkshopActionResult(new ScreenSteamWorkshop(), "Failed to remove " + uploadingType.toLowerCase() + "!", SteamResults.getMessage(result),
                        false);
        }
