...
public abstract class Tank extends Movable implements ISolidObject
{
    ...
    @TankBuildProperty
    @Property(category = appearanceBody, id = "color_skin", name = "Tank body skin", miscType = Property.MiscType.colorModel)
    public TankModels.TankSkin colorSkin = TankModels.tank;
    @TankBuildProperty
    @Property(category = appearanceTreads, id = "base_skin", name = "Tank treads skin", miscType = Property.MiscType.baseModel)
    public TankModels.TankSkin baseSkin = TankModels.tank;
    @TankBuildProperty
    @Property(category = appearanceTurretBase, id = "turret_base_skin", name = "Turret base skin", miscType = Property.MiscType.turretBaseModel)
    ...
    @TankBuildProperty
    @Property(category = general, id = "targetable", name = "Targetable", desc = "If disabled, AI-controlled tanks will not try to target this tank")
    ...
    public void update()
    {
        if (this.networkID < 0)
        {
            // If you get this crash, please make sure you call Game.addTank() to add them
            // to movables, or use registerNetworkID()!
    ...
    public void drawTank(boolean forInterface, boolean in3d, boolean transparent)
    {
        ...
        Drawing.drawing.setColor(teamColor[0] * this.glowModifier * this.glowIntensity, teamColor[1] * this.glowModifier * this.glowIntensity,
                teamColor[2] * this.glowModifier * this.glowIntensity, 255, 1);
        ...
        Drawing.drawing.setColor(this.color.red * (1 - Math.max(dmgFlash, healFlash)) + 255 * dmgFlash, this.color.green * (1 - Math.max(dmgFlash, healFlash)) + 255 * healFlash,
                this.color.blue * (1 - Math.max(dmgFlash, healFlash)), transparent ? 127 : 255, luminance);
        ...
        if (this.health > 1 && this.size > 0 && !forInterface)
        {
            ...
            {
                if (in3d)
                    drawing.drawModel(health_model, this.posX, this.posY, this.posZ + s / 4, size, size, s, this.orientation, 0, 0);
                else
                    drawing.drawModel(health_model, this.posX, this.posY, size, size, this.orientation);
    ...
    public double getAutoZoomRaw()
    {
        ...
        for (Movable m: Game.movables)
        {
            ...
            double boundedX = Math.min(Math.max(this.posX, Drawing.drawing.interfaceSizeX * 0.4), Game.currentSizeX * Game.tile_size - Drawing.drawing.interfaceSizeX * 0.4);
            double boundedY = Math.min(Math.max(this.posY, Drawing.drawing.interfaceSizeY * 0.4), Game.currentSizeY * Game.tile_size - Drawing.drawing.interfaceSizeY * 0.4);
            ...
            boolean isInSight = Ray.newRay(this.posX, this.posY, 0, 0, this).setTrace(Game.drawAutoZoom, true).isInSight(m);
