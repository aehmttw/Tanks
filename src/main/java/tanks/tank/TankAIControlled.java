...
public class TankAIControlled extends Tank implements ITankField
{
    ...
    public void handleBulletThreats()
    {
        ...
        if (m > Game.tile_size * 4 && avoidanceSeekOpenSpaces)
            dodgeOpenSpaces(direction);
        else
            dodgeBullet(m, direction, diff, nearest, nearestTarget);
    }

    private void dodgeBullet(double m, double direction, double diff, Bullet nearest, Ray nearestTarget)
    {
    ...
    private void dodgeOpenSpaces(double direction)
    {
    ...
    private void findBulletThreats()
    {
        avoid = false;
        toAvoid.clear();
        toAvoidDist.clear();
    ...
    private boolean shouldDodge(Bullet b, double dist)
    {
        ...
        return !(b.tank == this && b.age < 20) && !(this.team != null && Team.isAllied(b, this) && !this.team.friendlyFire)
                && b.dealsDamage && Math.abs(b.posX - this.posX) < Game.tile_size * distBox && Math.abs(b.posY - this.posY) < Game.tile_size * distBox
    ...
    public void lookAtTargetEnemy()
    {
        ...
        double a = this.getAngleInDirection(this.targetEnemy.posX, this.targetEnemy.posY);
