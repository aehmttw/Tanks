    ...
    public void update()
    {
            ...
            if (!Game.bulletLocked && !this.disabled && !this.destroy)
            {
                ...
                if (!shootStickEnabled || shootStickHidden)
                {
                    for (int i : Game.game.window.touchPoints.keySet())
                    {
                        ...
                        distSq = Math.pow(px - Drawing.drawing.gameToInterfaceCoordsX(this.posX), 2) + Math.pow(py - Drawing.drawing.gameToInterfaceCoordsY(this.posY), 2);
        ...
        if ((trace || lockTrace) && !Game.bulletLocked && !this.disabled && (Game.screen instanceof ScreenGame || Game.screen instanceof ScreenTitle))
        {
            ...
            if (i != null)
            {
                ...
                double range = b.lifespan > 0 ? b.lifespan * b.speed + this.turretLength * em().getAttributeValue(AttributeModifier.bullet_speed, 1) : 0;
                ...
                for (int j = 0; j < b.shotCount; j++)
                {
                    ...
                    for (int k = -gasSpread; k <= gasSpread; k++)
                    {
                        ...
                        Ray r = Ray.newRay(this.posX, this.posY, this.angle + baseOff + spreadOff, b.bounces, this).setSize(k != 0 ? b.size / 2 : b.size);
                        if (range > 0)
