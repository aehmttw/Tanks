...
public class Level {
	...
	public void loadLevel() {
		Registry.loadRegistry(Game.homedir);
		...
		for (int i = 0; i < Game.currentSizeX; i++) {
			for (int j = 0; j < Game.currentSizeY; j++) {
				Game.tiles[i][j] = new Color((int) (r + Math.random() * dr), (int) (g + Math.random() * dg),
						(int) (b + Math.random() * db));
			}
		}
			...
			for (double x = startX; x <= endX; x++) {
				for (double y = startY; y <= endY; y++) {
					...
					if (Game.graphicalEffects)
						col = new Color((int) (colorMul * (176 - Math.random() * 70)),
								(int) (colorMul * (111 - Math.random() * 34)), (int) (colorMul * 14));
					else
						col = new Color(87, 46, 8);

					Game.obstacles.add(new Obstacle(x, y, col));
					// Game.obstacles.add(new Obstacle(x, y, Color.getHSBColor((float) ((x + y) /
					// 100), 1, 1)));
				}
			}
		}
