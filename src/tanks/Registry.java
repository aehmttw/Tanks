...
public class Registry {
		...
		try {
			...
			while (in.hasNextLine()) {
				...
				if (tankLine[2].toLowerCase().equals("default")) {
					...
					if (!foundTank)
						Game.logger.println(new Date().toString() + " (syswarn) The default tank '" + tankLine[0]
								+ "' does not exist or cannot be loaded. ignoring.");
				} else {
					try {
						@SuppressWarnings("resource")
						ClassLoader loader = new URLClassLoader(new URL[] { new File(tankLine[3]).toURI().toURL() }); // super
																														// messy
						...
						e.printStackTrace();
						Game.logger.println(new Date().toString() + " (syswarn) error loading custom tank '"
								+ tankLine[0] + "'. ignoring.");
					}
				}
			}
			...
			Game.logger.println(
					new Date().toString() + " (syswarn) tank registry file is nonexistent or broken, using default. below is stack trace.");
	...
	public static void initRegistry(String homedir) {
		...
		try {
			...
			Game.logger.println(
					new Date().toString() + " (syserr) file permissions are screwed/broken! cannot initialize tank registry file.");
			System.exit(1);
		}
		try {
			...
			writer.println(
					"# Example custom tank entry: 'mytank,1,custom,C:\\Users\\potato\\.tanks.d\\MyTank.jar,com.potato.MyTank'");
			...
			Game.logger.println(new Date().toString() + " (syserr) something broke! cannot initialize tank registry. below is stack trace.");
			e.printStackTrace(Game.logger);
			System.exit(1);
		}
