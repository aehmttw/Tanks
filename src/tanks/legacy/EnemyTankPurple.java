...
public class EnemyTankPurple extends Tank
{
	...
	public void update()
	{
		if (!(this.s1.equals(State.shooting)))
		{ // still rotating if travelling
		...
		else
		{
			...
			// the below should be updating for the fire alogorithm.
			// cooldown=60ms.
			try
			{
				if (aimTarget.equals(Game.player))
				{
					...
					Thread shootThread = new Thread()
					{
						public void run()
						{
							...
							// set the angles and fire. set the state to firing
							// when the thread starts and back to finding when
							// it is done.
		...
		{
			Ray pathfinder = new Ray(this.posX, this.posY, this.angle, 8, this); // 8
																					// =
																					// big
																					// number!
			...
			try
			{
				if (driveTarget.equals(Game.player))
				{
					EnemyTankPurple me = this;
					/*
					 * it's just a small thing but I can't understand why I'm
					 * the enemy.
					 */
		...
		{
			// dont look around for where I am, just goddamn move as in the
			// thread
		}
	}
